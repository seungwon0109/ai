...
def _pe_has_dll_flag(path):
    """Detect IMAGE_FILE_DLL even when a sample uses a misleading extension."""
    try:
        with open(path, "rb") as stream:
            ...
            if stream.read(4) != b"PE\0\0":
                return False
            stream.seek(pe_offset + 22)
            characteristics_data = stream.read(2)
            return (
                len(characteristics_data) == 2
                and bool(struct.unpack("<H", characteristics_data)[0] & 0x2000)
            )
...
class Zip(Package):
    ...
    def execute_archive_member(self, root, file_name):
        file_path = os.path.join(root, file_name)
        if _pe_has_dll_flag(file_path) and not file_name.lower().endswith(".dll"):
            dll_name = f"{file_name}.dll"
            dll_path = os.path.join(root, dll_name)
            os.replace(file_path, dll_path)
            log.warning(
                "PE header marks %s as a DLL; routing renamed member through rundll32 as %s",
                file_name,
                dll_name,
            )
            file_name, file_path = dll_name, dll_path
    ...
    def start(self, path):
        ...
        try:
            ...
            seven_zip_path = self.get_7zip_path()
        ...
        if not file_name:
            ...
            for interesting_file in interesting_files:
                ret_list.append(self.execute_archive_member(root, interesting_file))

            return ret_list
        else:
            return self.execute_archive_member(root, file_name)
